...
function compileRender(templateSource, data, block) {
  ...
  return Promise.try(() => {
    ...
    const templateFunction = precompile(templateSource, {}).then(code => evaluate(code));
...
function compileParse(templateSource, block, data, callback) {
  ...
  compileRender(templateSource, data, block).then(output => process.nextTick(callback, null, output), err => process.nextTick(callback, err));
}

exports.compileRender = compileRender;
exports.compileParse = compileParse;
//# sourceMappingURL=compile-render.js.map
