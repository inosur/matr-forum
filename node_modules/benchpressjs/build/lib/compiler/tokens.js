...
HelperExpression.patterns = [`function\\.(${SimpleExpression.pattern})((?: *, *${simpleOrString})*)`, `(${SimpleExpression.pattern})(?:\\( *(${simpleOrString}(?: *, *${simpleOrString})*)? *\\))`];
...
OpenIf.patterns = [`<!-- IF (!?)(${Expression.pattern}) -->`, `{{{ ?if (!?)(${Expression.pattern}) ?}}}`];
OpenIf.priority = 60;
...
OpenIter.patterns = [`<!-- BEGIN (${SimpleExpression.pattern}) -->`, `{{{ ?each (${SimpleExpression.pattern}) ?}}}`];
OpenIter.priority = 50;
class Else extends Token {}

Else.patterns = ['<!-- ELSE -->', '{{{ ?else ?}}}'];
Else.priority = 40;
