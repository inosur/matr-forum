...
function Guard(path) {
  ...
  let last = c.CONTEXT;
  return t.callExpression(c.GUARD, [paths.reduce((out, p) => {
...
function UnsafeIter(branch) {
  ...
  return t.callExpression(t.functionExpression(null, [], t.blockStatement([t.variableDeclaration('var', [t.variableDeclarator(arr, Guard(branch.subject.path)), t.variableDeclarator(output, t.stringLiteral('')), t.variableDeclarator(c.LENGTH, t.memberExpression(arr, c.LENGTH)), t.variableDeclarator(c.INDEX, t.numericLiteral(0)), t.variableDeclarator(key), t.variableDeclarator(c.KEY)]), t.forStatement(null, t.binaryExpression('<', c.INDEX, c.LENGTH), t.updateExpression('++', c.INDEX), t.blockStatement([t.expressionStatement(t.assignmentExpression('=', key, c.INDEX)), t.expressionStatement(t.assignmentExpression('=', c.KEY, c.INDEX)), t.expressionStatement(t.assignmentExpression('+=', output, ConcatStringList(compile(branch.body))))])), t.returnStatement(output)])), []);
}
...
const transforms = {
  ...
  SimpleExpression(branch) {
    ...
    if (path === '@last') {
      return t.binaryExpression('===', c.INDEX, t.binaryExpression('-', c.LENGTH, t.numericLiteral(1)));
    }
  ...
  HelperExpression(branch) {
    if (options.unsafe) {
      return t.callExpression(t.memberExpression(c.HELPERS, t.stringLiteral(branch.helperName), true), compile(branch.args));
    }

    return t.callExpression(c.HELPER, [c.CONTEXT, c.HELPERS, t.stringLiteral(branch.helperName), t.arrayExpression(compile(branch.args))]);
  },
  ...
  OpenIter(branch) {
    ...
    const iter = t.callExpression(c.ITER, [Guard(branch.subject.path), t.functionExpression(c.EACH, [key, c.INDEX, c.LENGTH], t.blockStatement([t.variableDeclaration('var', [t.variableDeclarator(c.KEY, key)]), t.returnStatement(ConcatStringList(compile(branch.body)))]))]);
  ...
  EscapedMustache(branch) {
    return t.callExpression(c.ESCAPE, [transforms.RawMustache(branch)]);
  }
};
...
function compiler(parsed, opts) {
  ...
  return t.functionDeclaration(c.COMPILED, c.runtimeParams, t.blockStatement([t.returnStatement(ConcatStringList(compiled))]));
}
