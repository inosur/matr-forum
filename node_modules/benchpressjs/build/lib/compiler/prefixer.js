...
const fixIter = input => input.replace(/<!-- BEGIN ([^./][@a-zA-Z0-9/.\-_:]+?) -->([\s\S]*?)<!-- END \1 -->/g, (str, one, two) => {
  const t = fixIter(two);
...
input => input.replace(/<!-- BEGIN ([^./][@a-zA-Z0-9/.\-_:]+?) -->([\s\S]*?)<!-- END \1 -->/g, (str, one, two) => {
  const t = fixIter(two);
  return `<!-- BEGIN ${one} -->${t}<!-- END ${one} -->`;
}),
// wrap `@key`, `@value`, `@index` in mustaches
// if they aren't in a mustache already
input => input.replace(/(\{{1,2}[^}]+\}{1,2})|(<!--[^>]+-->)|(@key|@value|@index)/g, (str, mustache, block, lone) => {
  if (lone) {
    return `{${lone}}`;
  }
...
input => input.replace(/<!-- IF (?:function\.([@a-zA-Z0-9/._:]+)(?:\s*,\s*)?(.*?)) -->/g, (raw, helperName, argStr) => `<!-- IF function.${helperName}, @root${argStr ? `, ${argStr}` : ''} -->`)];
