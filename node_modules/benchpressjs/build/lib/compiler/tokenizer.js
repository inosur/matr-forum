...
function getTopLevelTokens() {
  return Object.keys(tokens).map(key => tokens[key]).filter(token => token.priority > 0).sort((a, b) => a.priority - b.priority);
}
...
function removeExtraCloses(output) {
  ...
  output.forEach((token, index) => {
    if (token.tokenType.startsWith('Open')) {
      expectedSubjects.push(token.subject && token.subject.path || token.test && (token.test.raw || token.test.path));
    } else if (token.tokenType === 'Close') {
